class ForestSafariRunner{
	public static void main(String[] safari){
	System.out.println("\nmain started\n");
	
	ForestSafari safari1=new ForestSafari();
	
	safari1.safariId=1;
	safari1.name="Jim cobat";
	safari1.location="India";
	safari1.hasTransport=true;
	safari1.noOfAnimals=345;
	safari1.isVisitationAllowed=true;
	
	
	
	ForestSafari safari2=new ForestSafari();

	safari2.safariId=2;
	safari2.name="Kaziranga";
	safari2.location="Assam";
	safari2.hasTransport=true;
	safari2.noOfAnimals=210;
	safari2.isVisitationAllowed=true;



	ForestSafari safari3=new ForestSafari();

	safari3.safariId=3;
	safari3.name="Ranthambore";
	safari3.location="Rajasthan";
	safari3.hasTransport=true;
	safari3.noOfAnimals=400;
	safari3.isVisitationAllowed=true;


	ForestSafari safari4=new ForestSafari();

	safari4.safariId=4;
	safari4.name="Sundarbans";
	safari4.location="West Bengal";
	safari4.hasTransport=false;
	safari4.noOfAnimals=500;
	safari4.isVisitationAllowed=false;



	ForestSafari safari5=new ForestSafari();

	safari5.safariId=5;
	safari5.name="Bandipur";
	safari5.location="Karnataka";
	safari5.hasTransport=true;
	safari5.noOfAnimals=300;
	safari5.isVisitationAllowed=true;


	ForestSafari safari6=new ForestSafari();

	safari6.safariId=6;
	safari6.name="Periyar";
	safari6.location="Kerala";
	safari6.hasTransport=false;
	safari6.noOfAnimals=220;
	safari6.isVisitationAllowed=true;



	ForestSafari safari7=new ForestSafari();

	safari7.safariId=7;
	safari7.name="Gir";
	safari7.location="Gujarat";
	safari7.hasTransport=true;
	safari7.noOfAnimals=310;
	safari7.isVisitationAllowed=true;



	ForestSafari safari8=new ForestSafari();

	safari8.safariId=8;
	safari8.name="Pench";
	safari8.location="Madhya Pradesh";
	safari8.hasTransport=true;
	safari8.noOfAnimals=275;
	safari8.isVisitationAllowed=false;



	ForestSafari safari9=new ForestSafari();

	safari9.safariId=9;
	safari9.name="Satpura";
	safari9.location="Madhya Pradesh";
	safari9.hasTransport=false;
	safari9.noOfAnimals=150;
	safari9.isVisitationAllowed=true;



	ForestSafari safari10=new ForestSafari();

	safari10.safariId=10;
	safari10.name="Nagarhole";
	safari10.location="Karnataka";
	safari10.hasTransport=true;
	safari10.noOfAnimals=330;
	safari10.isVisitationAllowed=true;


	ForestSafari safari11=new ForestSafari();

	safari11.safariId=11;
	safari11.name="Tadoba";
	safari11.location="Maharashtra";
	safari11.hasTransport=true;
	safari11.noOfAnimals=290;
	safari11.isVisitationAllowed=true;



	ForestSafari safari12=new ForestSafari();

	safari12.safariId=12;
	safari12.name="Simlipal";
	safari12.location="Odisha";
	safari12.hasTransport=false;
	safari12.noOfAnimals=180;
	safari12.isVisitationAllowed=false;



	ForestSafari safari13=new ForestSafari();

	safari13.safariId=13;
	safari13.name="Dudhwa";
	safari13.location="Uttar Pradesh";
	safari13.hasTransport=true;
	safari13.noOfAnimals=260;
	safari13.isVisitationAllowed=true;


	ForestSafari safari14=new ForestSafari();

	safari14.safariId=14;
	safari14.name="Bannerghatta";
	safari14.location="Karnataka";
	safari14.hasTransport=false;
	safari14.noOfAnimals=320;
	safari14.isVisitationAllowed=true;



	ForestSafari safari15=new ForestSafari();

	safari15.safariId=15;
	safari15.name="Valmiki";
	safari15.location="Bihar";
	safari15.hasTransport=true;
	safari15.noOfAnimals=240;
	safari15.isVisitationAllowed=false;



	ForestSafari safari16=new ForestSafari();

	safari16.safariId=16;
	safari16.name="Indravati";
	safari16.location="Chhattisgarh";
	safari16.hasTransport=true;
	safari16.noOfAnimals=310;
	safari16.isVisitationAllowed=true;



	ForestSafari safari17=new ForestSafari();

	safari17.safariId=17;
	safari17.name="Desert National Park";
	safari17.location="Rajasthan";
	safari17.hasTransport=false;
	safari17.noOfAnimals=150;
	safari17.isVisitationAllowed=false;



	ForestSafari safari18=new ForestSafari();

	safari18.safariId=18;
	safari18.name="Kanha";
	safari18.location="Madhya Pradesh";
	safari18.hasTransport=true;
	safari18.noOfAnimals=340;
	safari18.isVisitationAllowed=true;



	ForestSafari safari19=new ForestSafari();

	safari19.safariId=19;
	safari19.name="Anamalai";
	safari19.location="Tamil Nadu";
	safari19.hasTransport=false;
	safari19.noOfAnimals=205;
	safari19.isVisitationAllowed=false;


	ForestSafari safari20=new ForestSafari();

	safari20.safariId=20;
	safari20.name="Manas";
	safari20.location="Assam";
	safari20.hasTransport=false;
	safari20.noOfAnimals=190;
	safari20.isVisitationAllowed=false;

	safari1.getInfo();
	safari2.getInfo();
	safari3.getInfo();
	safari4.getInfo();
	safari5.getInfo();
	safari6.getInfo();
	safari7.getInfo();
	safari8.getInfo();
	safari9.getInfo();
	safari10.getInfo();
	safari11.getInfo();
	safari12.getInfo();
	safari13.getInfo();
	safari14.getInfo();
	safari15.getInfo();
	safari16.getInfo();
	safari17.getInfo();
	safari18.getInfo();
	safari19.getInfo();
	safari20.getInfo();


	
	System.out.println("\nmain ended");
	}
}